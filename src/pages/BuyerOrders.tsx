...
import { FileText, Plus, Clock, CheckCircle, Send, Archive, Truck, PackageCheck, XCircle, Cog } from 'lucide-react';
...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  ...
  processed: { label: 'Processed', color: 'bg-accent/10 text-accent-foreground border-accent/20', icon: Cog },
  on_delivery: { label: 'On Delivery', color: 'bg-primary/10 text-primary border-primary/20', icon: Truck },
  completed: { label: 'Completed', color: 'bg-success/10 text-success border-success/20', icon: PackageCheck },
  rejected: { label: 'Rejected', color: 'bg-destructive/10 text-destructive border-destructive/20', icon: XCircle },
  closed: { label: 'Closed', color: 'bg-muted text-muted-foreground border-muted', icon: CheckCircle },
};
    ...
    <PublicLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button asChild>
            <Link to="/products"><Plus className="h-4 w-4 mr-2" /> New Order</Link>
          </Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mb-4">Start by browsing our products</p>
