      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Track your quotation requests</p>
          </div>
          <Button asChild>
            <Link to="/rfq"><Plus className="h-4 w-4 mr-2" /> New Request</Link>
          </Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mb-4">Start by submitting your first quotation request</p>
            <Button asChild><Link to="/rfq">Submit RFQ</Link></Button>
          </div>
        ) : (
          <div className="space-y-4">
            {requests?.map(req => {
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="font-display font-semibold">{req.request_number}</h3>
                          <Badge className={`text-xs ${status.color}`} variant="outline">
                            <StatusIcon className="h-3 w-3 mr-1" />
                            {status.label}
                          </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground mb-1">{req.company_name} • {req.contact_person}</p>
                        {req.description && <p className="text-sm text-muted-foreground line-clamp-1">{req.description}</p>}
                      </div>
